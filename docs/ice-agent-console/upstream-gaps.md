...
> **2026-09-18 复核**：按「逐条对着**当前源码**核」的方式过了一遍 —— 引擎这段时间在快速
> 演进，这份清单里已经有 4 条**过时**（上游已修或前提变了）。改判的条目在标题上标了状态，
> 正文保留原始观察，随后追加「**现状**」一段。**别拿本文件当"上游现在还没做"的依据** ——
> 先看源码。
...
流式剧本（`看看出水实时流量`）因此改用数值轴——那本来就是 `appendData` 被设计出来服务的场景
（函数注释里写的"实时数据流专用"）。
...
## 4. 【已修 2026-09-18】`ChartEventName` 缺 `mark:drag` / `mark:dragend`
...
**现状（2026-09-18）**：**已修**。`ChartEventName` 与 `ChartEventPayloads` 都补上了这两个名字
（载荷类型 `ChartMarkData`），并且 `tests/chart/marks.test.ts` 里把回调参数从 `any` 收成了
`ChartMarkData`、另加一条类型层棘轮 —— 漏掉任何一个名字，`npm run types:check` 就会红
（此前"运行时能收、类型里没有"正是那种**不报错、只静默漏掉**的形态）。
...
## 8. 【观察，位置已变 2026-09-18】`ICEPanel` 不能当布局容器用（它是"卡片底座"）
...
**现状（2026-09-18）**：结论不变，**位置变了** —— `ICEGroup` 与整套布局器
（`ICEBoxLayout` / `ICEFlowLayout` / `ICEGridLayout` / `ICEBorderLayout` …）已经从
`ice-web-components` **下沉到引擎**（`ice-render/src/graphic/container/ICEGroup.ts`、
`ice-render/src/layout/`）。"布局容器不必是 Panel"这条现在属于引擎的能力，不是控件库的。
...
## 11. 【部分已修 2026-09-18】暗色主题：控件都能看，但层与层糊在一起 + 图表的提示框仍是白底
...
- 第 1 条（层与层明度差）：**照旧** —— 暗色 token 仍是 `#212529 / #2b3035 / #343a40`。
  它是设计取向，不是缺陷。
- 第 2 条（暗色图表提示框仍是白底）：**已修**。`ice-chart` 的暗色主题现在给了
  `tooltip.background = gray[800]`（亮色仍是白），而且 `'auto'` 以前被当成 `'light'` 的缺陷
  也一并改正了（`resolveChartTheme` 里 `theme === 'auto' && preferDark` 会真的走暗色）。
  也就是说这条当时的判断（"更接近 chartTheme，而不是引擎或控件库"）是对的，
  后来在 `ice-chart` 侧补齐了。

---

## 12. 【已修 2026-09-18】`EntityDesigner` / `WaterProcessDesigner` 没有程序化高亮原语
...
**现状（2026-09-18）**：**已修**，两条都修了。

1. `ice-entity-designer` 新增了程序化高亮原语：`FlowDesigner.setHighlights(ids, options)` /
   `highlight(id)` / `clearHighlights()` / `getHighlightedIds()`（实现见
   `src/designer/highlight.ts`）。描边环落在**工具层**，三条契约钉在单测里：
   **不进快照**、**不参与命中**（否则高亮会挡住被高亮的东西）、**图元移动/删除时自动跟随**。
   本工程已经把 `DiagramLayer.pointAt()` 切到它上面，"改 style 再自己置脏"那段绕法退休了。
2. 正文里那条 ⚠️ 也过时了：`WaterSymbol.applyPatch` 现在经 `setState` 走，
   而 `ICEComponent.setState` 会同时置 `dirty` 与 `ice.dirty` —— 不需要宿主再补一次。

---

## 13. 【已修 2026-09-18】`fitViewport()` 与 `dpr > 1` 不兼容
...
**现状（2026-09-18）**：**已修**，而且是按"两处一起修"做的 ——
这个方法有两个实现（`ice-entity-designer/src/flow/FlowDesigner.ts` 与
`ice-render-dsl/src/runtime/renderDsl.ts` 的同名函数），两处都改成：
优先用引擎的输入矩形（`ice.getInputRect()`，内容盒 = CSS 尺寸，与命中/坐标换算同口径），
拿不到时退回 `canvasWidth / dpr`。**明确不再退回 `canvasWidth` 本身** —— 那正是缺陷来源。
回归判据是"**同一画布、同一内容，只改 dpr，取景必须逐字段一致**"：
`FlowDesigner` 侧是单测（dpr=1 对照 dpr=2），DSL 侧是真实浏览器的 e2e
（再加一条"内容四角真的落在画布 CSS 区域内"）。两条都验证过"对着旧实现会红"。
...
**现状（2026-09-18）**：机制不变（这是对的），文档补了；**位置也变了** ——
过滤逻辑现在在 `DOMEventDispatcher.__isForeignCanvasTarget`（`src/event/DOMEventDispatcher.ts`），
拦截器本体只管"把原生事件广播给所有总线"。`ICE.setInputPassthrough()` 的注释里现在明确写了
它解决的是**另一块 canvas**，DOM 浮层要在自己的根上 `stopPropagation()`。
...
**现状（2026-09-18）**：行为不变，注释补上了 —— `FlowDesigner.remove()` 的文档里现在明确写了
"级联只覆盖引擎里那棵树；宿主若另有一份可序列化文档，请自行同步，别指望渲染层的副作用去补文档"，
并点了本仓 `pipesTouching` 这个正确做法。另外实现也扩了：删节点时会按 `this.nodes`
（而不是只判 `FlowNode`）识别"本设计器自己的节点"，UML / 状态机 / 甘特删节点不再漏摘监听与悬空连线。

---

## 16. 【已修 2026-09-18】管线标注的位置固定在折线顶点上，没有偏移手段 → 标注互相压住
...
**现状（2026-09-18）**：**已修**。引擎给连线标签加了偏移入口
`style.label.offset = [dx, dy]`（本地坐标 / 世界单位），落在 `ICEPolyLine.__labelMetrics()` 里，
于是**画布、包围盒（脏区擦除盒与离屏缓存范围）、SVG 导出**三处共用同一份度量、不会漂移；
非法值（长度不是 2 / 含非有限数）一律当没写，不抛错。

这条是整份清单里**唯一"应用层无论怎么努力都留一点瑕疵"**的地方，所以它被优先修掉：
有了它，应用层就能做"标注沿法向错开"这类避让，而不是只能靠改数据把个别冲突挪走。
（本工程尚未改数据去用这个能力 —— 现状 43 处擦碰属于"可读但不精致"，
要不要逐条避让是版面决策，留给下一轮。）
...
| 10 | 请求（文档已补 09-18） | 控件宽度没有"父级拉满"的传导 | 否 —— DSL 层兜住 + `ICEForm` / `layout.md` 写明了 `stretch` 的作用边界 |
| 11 | 部分已修 09-18 | 暗色层间明度差小（设计取向）+ 图表提示框白底（缺陷） | 否 —— 提示框已修；明度差仍是待定的设计取向 |
| 12 | **已修 09-18** | 没有程序化高亮原语 | 否 —— 新增 `setHighlights()` / `highlight()` / `clearHighlights()`，本工程已切换过去 |
| 13 | **已修 09-18** | `fitViewport()` 与 `dpr > 1` 不兼容 | 否 —— 两处实现都按 CSS 尺寸算，单测 + 真实浏览器 e2e 双向验证 |
| 14 | 观察（位置已变） | 事件拦截器不过滤非 canvas 的浮层 | 否（浮层自己 `stopPropagation`）—— 过滤逻辑在 `DOMEventDispatcher`，文档已补 |
| 15 | 观察（注释已补 09-18） | `FlowDesigner.remove()` 的级联只覆盖画面，不覆盖外部文档 | 否（补丁里显式列连带删除） |
| 16 | **已修 09-18** | 管线标注钉在折线顶点上、无偏移手段 | **否 —— 唯一那条"应用层解决不了"的已修**：`style.label.offset = [dx, dy]` |

**结论（2026-09-18 复核后重写）**：

- **真正"应用层解决不了、必须上游改"的只有一条 —— 第 16 条，已经修了**（`style.label.offset`）。
- **本轮一共改判 6 条**：第 4、12、13、16 条**已修**；第 11 条**部分已修**（提示框修了、
  层间明度差是设计取向照旧）；第 7、10、14、15 条属于"文档/位置补正"（第 7 条仍坚持 B 路线、
  不改引擎；后三条补了注释与位置）。
- **另有 3 条要当心"记录过时"**：第 8 条（`ICEGroup` 与布局器已下沉到引擎）、
  第 12 条正文里的 ⚠️（`applyPatch` 现在会置脏）、第 14 条（过滤逻辑搬到了 `DOMEventDispatcher`）。
  这一类变化**不会有人来通知你**，只能每次改之前对源码核一遍。
- 作为一次对 ICE 家族对外接口的真实集成测试，结论仍然站得住：**接口够用**；
  需要上游动的，绝大多数是"契约没写下来"（文档），少数是真缺口（渲染原语与 dpr 口径）。

**第 11 条的"层与层糊在一起"仍是唯一一条悬着的设计取向**（不是缺陷）：暗色面板的
`background / surface / elevated` 三档明度差只有几个百分点。要不要改、改成什么基准，
是产品决策；本工程默认 light，`?theme=dark` 留着复查。
