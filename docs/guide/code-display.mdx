...
**4. 用法** —— 语言写 `jsx live`，`title` 可选；完整可运行示例见 [可编辑运行](/docs/examples/live-playground)：
...
> 已通过 swizzle `@theme/ReactLiveScope` 把 `ICE` 注入 live 块作用域，可直接编辑 ICE 代码；不想引入可编辑能力时，仍可用 `<IceCanvas>` 展示固定示例（见[形状 · 实时示例](/docs/examples/shapes-live)）。
