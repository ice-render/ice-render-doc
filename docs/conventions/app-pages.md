---
description: "应用层「一个页面怎么写」的家族契约：宿主驱动页面的主链、onUpdate 的调用时机、稳定结构与可变内容的边界、四个入口怎么选、验收清单与常见坑。"
keywords:
  - "应用层约定"
  - "页面写法"
  - "一页一个类"
...
> 状态：**2026-09-17 立**的家族级约定。正文源头在 `ice-web-components/docs/guides/app-pages.md`
> （库仓里那份是单一来源，本页是它的站点镜像）；容器本身的契约见
> [画布内布局](../ice-web-components/guides/layout) 第六节。
>
> 为什么值得写下来：这一层以前靠口口相传，12 个页面能写出 12 种结构。2026-09-17 家族把应用层
> 统一到「一页 = 一个类」并加了棘轮（各仓 `tests/**/*onvention*.test.ts`），本页是那份契约的正文。
...
| 成员顺序 | `static 常量/字段 → static 方法 → 实例字段 → 构造函数 → 访问器 / 实例方法`（正则 `S*T*F*C*(A|M)*`） | [成员顺序契约](./member-ordering) |
...
| 你要做的东西 | 用哪一层 | 从哪抄 |
|---|---|---|
| 叶子控件（按钮 / 输入 / 表格 / 弹窗 / 日期…） | `ice-web-components`（`ICEWidget` 子类） | [写一个自己的组件](../ice-web-components/guides/custom-components)、`examples/custom-component.html` |
| 整页结构（顶栏 / 侧栏 / 卡片 / 多个控件） | `ICEContainer` 子类 —— **本页** | `ice-smart-water/src/view/pages/*.ts`（12 页） |
...
> 跨仓铁律（渲染 / 序列化 / 事件 / i18n 边界 / 动画等）见引擎仓 `AGENTS.md`；本页只管
> "应用页面怎么写"这一层。
