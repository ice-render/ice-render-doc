...
# ice-web-components-dsl · 表单 JSON DSL（当前 v0.4.0）

:::info 🔌 DSL 层 · 应用级 · 让 AI Agent 用一份扁平 `fields[]` 编译出 ICE 表单（当前 v0.4.0）
...
**ice-web-components-dsl 是 `ice-web-components` 的 agent 友好表单层（当前 `ice-web-components-dsl` v0.4.0）。** 它守住三条明确的增量，而不是「把 `new ICEForm(...)` 换个写法」：
