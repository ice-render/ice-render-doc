---
sidebar_position: 1
description: "ice-chart（Canvas 图表库）：基于 ICE Render 引擎的高性能图表组件，支持折线 / 柱状 / 饼图等，数据驱动、可交互、可序列化。"
keywords:
  - "ice-chart"
  - "Canvas 图表"
  - "图表库"
...
# ice-chart · 交互式图表库（当前 v0.30.12）

:::info 🧩 应用层 · 基于 ice-render 引擎 v4.3.0 封装（当前 v0.30.12）
本页属于 ice-render 家族的**应用层**产品：图表层只声明「数据 → 像素 → 语义事件」，命中测试与事件派发全部交给引擎内核。想了解底层引擎本身，请看左侧「ice-render 引擎」分组。
:::

**ice-chart 是构建在 ice-render 引擎之上的交互式图表库（当前 `@damoqiongqiu/ice-chart` v0.30.12）。** 它不是「把数据画成图」的又一个图表库——命中测试、事件派发、嵌套坐标系、脏矩形局部重绘**全部交给 ice-render 引擎**，图表层只负责把「数据 ↔ 像素 ↔ 语义事件」这三件事打通。
...
## 一个完整的实时大屏例子（就是仓库里的 `examples/dashboard.html`）

下面这个例子**直接由文档静态托管的 ice-render + ice-chart UMD 运行时在文档页里画出**（无需联网）：运营监控大屏（流量、延迟、水位、拓扑、桑基、热力等一整屏图表），所有交互（悬停十字准星、框选缩放、图例联动、键盘导航）都由引擎内建。

<LiveExample
  html="/ice-chart/dashboard.html"
  bundles={[
...
| 能力 | 说明 |
| --- | --- |
| 图表类型 | 折线 / 面积 / 柱状 / 堆叠柱 / 饼 / 玫瑰 / 雷达 / 仪表盘 / 漏斗 / 水位球 / 散点 / 气泡 / 箱线 / 桑基 / 关系图 / 树图 / 热力 / 时序等，均基于 ice-render 图元 |
...
:::info 0.30.5 → 0.30.12 值得注意的点（性能这条线的收口）
- **一次更新只跑一遍归一化**：视窗（缩放 / 平移 / 联动）只落在「域」上，不再为它重跑整条流水线。
  表达式绘图与等比坐标例外（它们的采样 / 跨度由视窗决定，照旧整条重跑）。
- **类目域改成增量维护**：滚动窗口的「删头 / 补尾」各 O(delta)，照惰性原始点那套**绝对序号**口径，
  **逐项验过才认**、判不中就退回全量；多系列共用同一批 x 时复用同一张表。
- **`chart.batch(fn)`：同帧批合并** —— 一帧里对多个系列的更新合成**一次**流水线。
  ```ts
  chart.batch(() => {
  ...
  ```
  语义：出批时图表已是新状态（同步）；批内不发布 `data:change`、出批后按调用顺序补发；
  批内抛异常也 flush。
- **只换视窗不重建点集**：平移 / 缩放不再重建 10 万个 `DataPoint`（按数据数组身份 + 长度 + 解析规则复用）。
- 实测（10 万点普通折线，真机 Chromium）：`setData` **6.2 → 3.6ms**、真滑窗 **10.5 → 4.2ms/tick**、
  多系列逐系列 `setData` **45.4 → 10.4ms/tick**、只换视窗 **3.4 → 1.0ms/tick**。
- 顺带修掉一条真 bug：类目域的缓存键原来只按「点数 + 首末 x」记，
  **中间被换掉会命中过期的域**（类目轴上凭空少 / 多一个类目），现在按数据数组身份记。

机制与边界见仓库里的架构文档：[`docs/README.md`](https://github.com/ice-render/ice-chart/blob/main/docs/README.md)（10 篇，带目录与编号）。
:::
...
<LiveExample html="/ice-chart/large-data-virtual-series.html" height={520} note="100 万点散点 / 折线（列存 + 环形缓冲）、100 万格热力矩阵、以及把同一份列交给引擎虚拟层" />

### 十万级：虚拟化前后对照

<LiveExample html="/ice-chart/large-data-virtual.html" height={520} note="同一份数据的三种存法（普通 / 列存 / 惰性原始点）在构建、堆、命中和滚动上的实测对照" />
...
- 架构与机制文档（仓库内，10 篇带目录与编号）：[docs/README.md](https://github.com/ice-render/ice-chart/blob/main/docs/README.md)
