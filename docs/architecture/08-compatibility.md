...
| `OffscreenCanvas` + Worker | **部分采用（阶段一）** | 引擎现在**能作为库直接跑在 worker 里**（取根 `globalThis` + `createOffscreenCanvas` 的 `OffscreenCanvas` 分支），原型不再需要宿主伪造全局。**仍未做**：场景/状态跨线程同步、输入转发、字体图片下发（即"把引擎正式移植进 worker"本身）。 |
