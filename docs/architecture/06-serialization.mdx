...
`zIndex` 是**进文档**的字段（`NON_SERIALIZABLE_KEYS` 里没有它）。默认值是 **`'auto'`**（`Z_INDEX_AUTO`，
所以写法就一条规则：**默认值不写、显式值原样写**（`Serializer.__encodeChildren`）。

- **默认（`'auto'` / 等价于 0）不写**：读回时就是默认值，文档里不会出现一整片 `zIndex`；一份"没有手工调过次序"
  的文档完全不带 `zIndex`；
...
### 格式升到 **v2** + `zIndex` 迁移（2026-09-19）

同一个数字在 v1 / v2 里含义不同：v1 的 `zIndex` 是"构造顺序计数器"留下的数字（v2 把它当
**应用自己钉的正数**）。所以格式版本升到 `SERIALIZATION_VERSION = 2`，并在
`SERIALIZATION_MIGRATIONS` 里挂了一条 **v1 → v2** 的迁移（`migrateZIndexToV2`）：

- 对**每个兄弟组**按 v1 口径（数字升序、平手按加入顺序）排出绘制次序，再按 v2 口径重编号成
  `-(m-1) … 'auto'`（最上面那个落回 auto 层，默认值直接删字段）→ **次序逐项不变**；
- 整组本来就全是 0 / 没写（v1 下就是纯加入顺序）时**不动它** —— 那种文档在 v2 下本来就正确；
- 不做这条迁移的后果是实测过的：旧文档（198~200）+ 新建组件 → **新建的沉到最下面**，
  也就是 v1 时代那个"新建的图元看不见"换个入口回来。

⚠️ **降级读会响亮失败**：2.16.0 拿到 `zIndex: 'auto'` 会按数字比较（`NaN`）静默乱序，
现在它会报「不支持的反序列化版本」。这是有意的 —— 宁可拒绝，也不要画错。
（代价：v1 文档里"钉在 9999 的浮层"迁移后落回 auto 层、不再有钉子效力；v1 本来也没这个保证 ——
计数器涨过 9999 之后它同样会被新内容盖住。）

回归：`tests/graphic/z-index-order.test.ts`（默认不写 / 显式原样 / 负数往返次序不变 / 加载后新建仍在最上）、
`tests/persistence/zindex-v2-migration.test.ts`（旧文档次序不变 + 新建仍在最上 / 次序乱序也保持 /
全 auto 不动 / v2 文档不迁移 / 版本号抬到最新）、
`tests/persistence/type-registry.test.ts`（迁移按 `to` 升序逐级执行）、
...
<LiveExample html="/ice-render/examples/serialization.html" height={460} note="序列化生成 JSON；清空后从同一份 JSON 反序列化重建，验证往返一致性" />
