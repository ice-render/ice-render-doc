...
| 2026-09-12 | P1-3 导出与互操作 · **SVG 导出**（`ice.toSvg` / `exportSvg`，与画布同一套绘制命令流）+ **路径命令流底座**（`src/cross-platform/Path2DRecorder.ts`，为 SVG / 服务端出图打底） | ✅ 已完成（1.4.2；补丁：`closePath` 按位置进命令流、连线标签进导出、实心端点箭头导出为填充路径） |
