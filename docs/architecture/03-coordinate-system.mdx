...
- **`__localBox()`：本地盒的「唯一来源」** —— 返回 `[x0, y0, x1, y1]`（未变换、未减原点），
  ...
  快照盒退化 → 脏矩形局部重绘漏画折线，详见 [13](gap-analysis) §4.4）。
...
<LiveExample html="/ice-render/examples/coordinate.html" height={420} note="移动父组，观察子组件 composedMatrix 平移分量如何随嵌套坐标变化（局部 left/top 不变）" />
