...
## 事件来源与 `target` 契约（2026-09-19）

使用者最容易踩的两件事都在"事件从哪来"上，所以 `ICEEvent` 把它们显式化了：

| 字段 | 取值 | 含义 |
|---|---|---|
| `evt.source` | `'canvas'` | 原始输入发生在**画布内**（指针 / 鼠标 / 触摸 / 滚轮 / 键盘）。有命中组件时 `target` 是它；点在画布空白处 `target` 为 `null`，事件照旧到总线 |
| | `'window'` | 原始输入发生在**画布之外**（工具栏按钮 / 页面空白）。原始输入监听挂在 `window` 上（拖拽移出画布也要跟手），这些事件同样会被转发到总线 |
| | `'engine'` | 代码创建：`trigger()` / `dispatchEvent()` / 引擎内部派发 |
| `evt.target` | 组件 \| `null` | **永远不是 DOM 元素**：要么是命中的组件，要么是 `null`；原始 DOM 元素在 `evt.originalEvent.target` |
| `evt.eventPhase` | `2` / `3` / `0` | 命中段 / 祖先段 / 传播结束（**总线那一段恒为 `0`** —— 总线是传播终点，不在任何段里） |

两条实现约束（改这块时别破坏）：

1. **`ICEEvent` 构造函数不从传入对象拷贝身份字段**（`NON_COPYABLE_FIELDS`：`target` / `currentTarget` /
   `srcElement` / `eventPhase` / `source`）。这些字段的语义是"引擎的派发结果"，不是事件源的属性：
   DOM 事件的这些字段一旦被平铺进来，`evt.target` 就会变成 `HTMLButtonElement`（应用只能靠
   `instanceof` 猜），`eventPhase` 还会出现"同一个字段两种含义"（命中组件时 `0`、无命中时沿用 DOM 的 `3`）。
2. **总线段触发前必须把相位归零**（`DOMEventDispatcher.__dispatch`）。组件链末尾已经归零，
   但"没有命中组件"的事件走不到组件链，不显式归零就会沿用原始 DOM 的 `BUBBLING_PHASE(3)`。

⚠️ **已知行为（本次刻意不改，要改先评估）**：画布外的输入**仍然会做命中测试** —— 坐标按画布矩形
（`clientX - rect.left`）换算，工具栏按钮恰好落在画布右侧时，换算结果可能落在画布内并命中组件
（表现为"点按钮顺带选中了画布元素"）。归属信息已由 `source` 明确给出，应用过滤即可；
要不要"画布外输入干脆不做命中"是语义级决策（会牵动键盘转发与 HTML 浮层的既有行为）。

回归：`tests/event/event-source.test.ts`（8 条，含这条"已知行为"的钉桩——改的时候会红）。
...
- 各 Manager 通过 `on/off` 订阅感兴趣的事件（见 [01 运行时](runtime)）。
...
- 做位移增量必须用**值快照**，不能拿上一次的 rect 对象引用做差 —— 某些运行时（测试桩 / headless）
...
<LiveExample html="/ice-render/examples/event.html" height={460} note="点击 / 悬停图形，日志实时打印 ICEEventTarget 派发过程；可挂起 click 验证 suspend 语义" />
