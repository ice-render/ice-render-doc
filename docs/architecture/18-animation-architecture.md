...
- 现状 `FULL_FALLBACK_DIRTY_RATIO = 0.2` 用的是"脏组件数 / 可见组件数"；动画的脏区**面积**往往很小，
  却按个数被判死刑（实测 1 万图元里 1500 个动 = 4.7ms 局部，2100 个动 = 12.7ms 全量）。
- 目标：以**脏区面积占比**为主判据（`FULL_FALLBACK_AREA_RATIO` 已有），
...
| 动画帧的脏区占比 | 走局部重绘；超过门限才允许全量 | 计数门换成面积门后重测 |
...
| 10,000 图元中 2,100 个在动（21% 脏） | 12.7ms/帧（回退全量） | **≤6ms/帧**（走局部重绘） |
...
- **最大风险是像素一致性**：写值通道与面积门都直接触碰"脏矩形 / 离屏缓存与全量逐像素一致"这条契约。
