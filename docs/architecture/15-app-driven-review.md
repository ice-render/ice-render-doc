...
```mermaid
flowchart LR
  ENG["ice-render 引擎（复盘时 1.4.1；本页含 2026-09-19 复核）"] --> OK
  ENG --> WIP
  subgraph OK["✅ 足够 / 很好"]
    O1["渲染原语与绘制"]
    O2["事件与命中"]
    ...
    O4["脏矩形 + HiDPI + 视口"]
    O5["序列化 / 插件 / a11y 原语"]
    O6["栈序 zIndex（2.13.0 起只在兄弟之间比较）"]
    O7["命中与容器语义（与渲染顺序同源，可逐组件关）"]
  end
  subgraph WIP["🟡 够用但要补协议"]
    W1["裁剪与滚动（滚动是组件级实现）"]
    W2["布局 measure/arrange 协议"]
    W3["文本（组件没用满 IME）"]
  end
```
...
> 语义上就是"每层一个 stacking context"（见 [02](component-model) 与 [04](rendering-performance)）。
