...
| 约束 | 阈值 | 依据 |
|---|---|---|
| 同时动画的**矩形 / 路径**类组件 | ≤ 10,000（≈60fps 边缘） | ② |
| 同时动画的**文本**组件 | ≤ 100~200（再多必须先做写值通道优化，⑤） | ④ |
| 脏组件占比 | ≤ 20%，否则必然整屏重绘 | ③ |
...
1. **动画写值专用通道**：纯平移只置 `dirty`、不置 `paramsDirty` → 文本动画才能走位图纯平移复用
   （⑤ 实测 34.8ms → 2.6ms 量级）；
2. **把「计数门」换成「面积门」**（或对"纯平移 + 已缓存"直接放行）→ 消掉 ③ 的断崖；
3. **帧率分级 + 空闲停帧**：次要动画按 30fps 推进；没有动画且无脏帧时停 rAF
