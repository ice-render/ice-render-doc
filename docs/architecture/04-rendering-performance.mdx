...
| 场景含折线（`isLine`）且其带宽盒较大 | 富场景下会**稳定回退全量** | 折线包围盒修复为真实值后属「clip 会切断描边抗锯齿」的风险类别。这是**正确的保守行为** —— 此前「能走局部重绘」恰恰是因为盒子退化、漏画了折线（见 [13](gap-analysis) §4.4） |
...
- 缓存决策（`ObjectCache.render`）：
  ...
  - dirty → 先 `refreshParams()` **按需**刷新派生状态（只有自身派生参数变脏时才重算点集 / 文本量测；
    ...
    （`composeMatrix()` 对 dots 的平移已改为幂等，不再要求每次 compose 前都重算 dots，见 [02](component-model)）
...
2. **优化不得破坏语义**：`calcAbsoluteLinearMatrix` 仍要实时重算祖先（见 [03](coordinate-system)），零分配只是复用存储、不是复用缓存值。
...
直接塌缩成并集盒 → 交给面积阈值回退全量，见 [18 §3.3](animation-architecture)）。
...
3. **OffscreenCanvas / Worker**：把光栅化搬离主线程（成本最高，见 [10](worker-offscreen)）。
...
1. ~~**把「计数门」换成「面积门」**（或对"纯平移 + 已缓存"直接放行）~~ ——
   **2026-09-13 实测后否决**（结论与数据见
   [18 · 动画机制 §3.3](animation-architecture)）：脏区一散开，并集盒必然撞 0.35 的面积阈值，
...
2. ~~**帧率分级 + 空闲停帧**~~ —— **✅ 已落地（2026-09-13，详见
   [18 §3.4](animation-architecture)）**：`FrameManager` 改成按需续帧（宿主用 `needsFrame()` 回答
...
3. ❌ **OffscreenCanvas / Worker**（见 [10](worker-offscreen)：目前只有设计 + 最小可行性原型，
   引擎尚未正式移植到 worker）。
...
完整配方与约束见 [18 · 动画机制 §3.1](animation-architecture#31-分层渲染静态层--动画层)。

> 这几条的**目标架构、约束与验收指标**已整理成 [18 · 动画机制](animation-architecture)
...
<LiveExample html="/ice-render/examples/rendering.html" height={460} note="300 静态点 + 1 动画点；切换 full / dirty-rect，观察 FPS 与「本帧跳过组件数」差异" />
