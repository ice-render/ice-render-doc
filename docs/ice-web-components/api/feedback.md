...
源码：``src/components/ICEDrawer.ts``
...
源码：``src/components/ICETooltip.ts``
...
源码：``src/components/ICEPopover.ts``
...
源码：``src/components/ICEPopconfirm.ts``
...
结果页：状态图标 + 标题 + 副标题 + 操作按钮组。 用于提交成功/失败、404、无权限等场景。

源码：``src/components/ICEResult.ts``
...
源码：``src/components/ICESkeleton.ts``
...
源码：``src/components/ICESpin.ts``

**构造参数** `ICESpinOptions` — 加载指示器：一段圆弧绕中心旋转。
...
源码：``src/components/ICEFloatButton.ts``
