...
# ice-web-components · Canvas 原生 UI 组件库（当前 v1.22.4）

:::info 🧩 应用层 · 基于 ice-render 引擎 v4.2.0 封装（当前 v1.22.4）
本页属于 ice-render 家族的**应用层**产品：所有 UI 像素都由引擎绘制，组件库只在其上收敛出 Swing 风格 Canvas 组件 API。想了解底层引擎本身，请看左侧「ice-render 引擎」分组。
:::

**ice-web-components 是构建在 ice-render 引擎之上的 Swing 风格 Canvas UI 组件库（当前 `ice-web-components` v1.22.4）。** 它把所有 UI 像素——按钮、输入框、表格、弹窗、乃至焦点环与阴影——**全部画在一个 `<canvas>` 上**：没有 DOM widget，没有 CSS 框架，每一像素都由引擎绘制。
