...
# Entity Designer · ER 图设计器（当前 v0.2.5）

:::info 🧩 应用层 · 基于 ice-render 引擎 v2.3.0 封装（当前 v0.2.5）
...
域包 = **一个领域的记法 + 应用层 + 语义校验**。除默认的 ER 之外，现已落地 7 个——仓库 `examples/` 下都有完整示例页。下面每一个都是**文档内实时运行的 live demo**（不是截图、不是外链），直接拖、直接连、直接看 Schema 落地：
