...
- **SVG**：`result.designer.toSvg(options)`（flowchart / BPMN 等）或 `IED.exportSvg(result.ice, options)`。`options = { area: 'content' | 'viewport', padding, scale, background, includeTools }`。
- **BPMN 2.0 XML**：`IED.toBpmnXml(result.designer)`——含 BPMNDI（互操作格式，非执行模型），用 `result.designer.validateBpmn()` 做语义检查后再导出。
- **PlantUML**：`IED.toPlantUml(result.designer)`（UML 类图 / 状态图文本互操作），并支持 `IED.fromPlantUml(text, designer)` 反向导入。
