...
# ice-chart-dsl · 图表 JSON DSL（当前 v0.6.0）

:::info 🔌 DSL 层 · 应用级 · 让 AI Agent 用「一张表 + encoding」编译出 ChartOption（当前 v0.6.0）
...
**ice-chart-dsl 是 `ice-chart` 图表库的 agent 友好层（当前 `@damoqiongqiu/ice-chart-dsl` v0.6.0）。** 它守住三条明确的增量，而不是「把 option 换个写法」：
